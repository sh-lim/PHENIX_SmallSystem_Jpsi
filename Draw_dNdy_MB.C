...
void Draw_dNdy_MB(){
	...
	//Theory curves
	TFile *infile_th0_HeAu = new TFile("RHeAu_MB_rap_predictions.root","read");
	TFile *infile_th0_pAu = new TFile("RpAu_MB_rap_predictions.root","read");
	TFile *infile_th0_pAl = new TFile("RpAl_MB_rap_predictions.root","read");

	TGraphAsymmErrors *gth_epps_HeAu[narm];
	TGraphAsymmErrors *gth_ncteq_HeAu[narm];
	gth_epps_HeAu[0] = (TGraphAsymmErrors*)infile_th0_HeAu->Get("RHeAu_epps_MB_rap_predictions_bkwd");
	gth_epps_HeAu[1] = (TGraphAsymmErrors*)infile_th0_HeAu->Get("RHeAu_epps_MB_rap_predictions_fwd");
	gth_ncteq_HeAu[0] = (TGraphAsymmErrors*)infile_th0_HeAu->Get("RHeAu_ncteq_MB_rap_predictions_bkwd");
	gth_ncteq_HeAu[1] = (TGraphAsymmErrors*)infile_th0_HeAu->Get("RHeAu_ncteq_MB_rap_predictions_fwd");

	TGraphAsymmErrors *gth_epps_pAu[narm];
	TGraphAsymmErrors *gth_ncteq_pAu[narm];
	gth_epps_pAu[0] = (TGraphAsymmErrors*)infile_th0_pAu->Get("RpAu_epps_MB_rap_predictions_bkwd");
	gth_epps_pAu[1] = (TGraphAsymmErrors*)infile_th0_pAu->Get("RpAu_epps_MB_rap_predictions_fwd");
	gth_ncteq_pAu[0] = (TGraphAsymmErrors*)infile_th0_pAu->Get("RpAu_ncteq_MB_rap_predictions_bkwd");
	gth_ncteq_pAu[1] = (TGraphAsymmErrors*)infile_th0_pAu->Get("RpAu_ncteq_MB_rap_predictions_fwd");

	TGraphAsymmErrors *gth_epps_pAl[narm];
	TGraphAsymmErrors *gth_ncteq_pAl[narm];
	gth_epps_pAl[0] = (TGraphAsymmErrors*)infile_th0_pAl->Get("RpAl_epps_MB_rap_predictions_bkwd");
	gth_epps_pAl[1] = (TGraphAsymmErrors*)infile_th0_pAl->Get("RpAl_epps_MB_rap_predictions_fwd");
	gth_ncteq_pAl[0] = (TGraphAsymmErrors*)infile_th0_pAl->Get("RpAl_ncteq_MB_rap_predictions_bkwd");
	gth_ncteq_pAl[1] = (TGraphAsymmErrors*)infile_th0_pAl->Get("RpAl_ncteq_MB_rap_predictions_fwd");

	// Theory curves Ramona
	...
	gth1_epps_pAl[0] = (TGraphAsymmErrors*)infile_th1_pAl->Get("RpAl_epps16_ramona_rap_MB_bkwd");
	gth1_epps_pAl[1] = (TGraphAsymmErrors*)infile_th1_pAl->Get("RpAl_epps16_ramona_rap_MB_fwd");
	
	TGraphAsymmErrors *gth1_epps_pAu[narm];
	gth1_epps_pAu[0] = (TGraphAsymmErrors*)infile_th1_pAu->Get("RpAu_epps16_ramona_rap_MB_bkwd");
	gth1_epps_pAu[1] = (TGraphAsymmErrors*)infile_th1_pAu->Get("RpAu_epps16_ramona_rap_MB_fwd");
     
	TGraphAsymmErrors *gth1_epps_HeAu[narm];
	gth1_epps_HeAu[0] = (TGraphAsymmErrors*)infile_th1_HeAu->Get("RHeAu_epps16_ramona_rap_MB_bkwd");
	gth1_epps_HeAu[1] = (TGraphAsymmErrors*)infile_th1_HeAu->Get("RHeAu_epps16_ramona_rap_MB_fwd");
	...
	int N1 = gth1_epps_pAl[0]->GetN();
	cout << " N Vogt " << N1 << endl;
	double x1[3], y1[3], y1_new[3], y1_new_up[3], y1_new_dn[3];
	double y1_abs[3], y1_abs_err[3];
	for(int i=0;i<N1; ++i)
	  {
	    int val = gth1_epps_pAl[0]->GetPoint(i, x, y);
	    x1[i] = x;
	    y1[i] = y;
	    
	    cout << " i " << i << " x1 " << x1[i] << " y " << y1[i] << endl;
	  }
	
	y1_abs[0] = mod_pal_bu_only[0] - (mod_pal_bu_only[1] - mod_pal_bu_only[0]) / (x_new[1] - x_new[0]) * (x_new[0] - x1[0]);
	y1_abs_err[0] = mod_pal_bu_only_err[0] - (mod_pal_bu_only_err[1] - mod_pal_bu_only_err[0]) / (x_new[1] - x_new[0]) * (x_new[0] - x1[0]);

	y1_abs[1] = mod_pal_bu_only[1] + (mod_pal_bu_only[2] - mod_pal_bu_only[1]) / (x_new[2] - x_new[1]) * (x_new[2] - x1[1]);
	y1_abs_err[1] = mod_pal_bu_only_err[1] + (mod_pal_bu_only_err[2] - mod_pal_bu_only_err[1]) / (x_new[2] - x_new[1]) * (x_new[2] - x1[1]);

	y1_abs[2] = mod_pal_bu_only[3] + (mod_pal_bu_only[3] - mod_pal_bu_only[2]) / (x_new[3] - x_new[2]) * (x1[2] - x_new[3]);
	y1_abs_err[2] = mod_pal_bu_only_err[3] + (mod_pal_bu_only_err[3] - mod_pal_bu_only_err[2]) / (x_new[3] - x_new[2]) * (x1[2] - x_new[3]);

	for(int i=0;i<N;++i)
	  {
	    cout << " i " << i << " x_new " << x_new[i] << " mod_pal_bu_only " << mod_pal_bu_only[i] << endl; 
	  }
	for(int i=0;i<N1;++i)
	  {
	    cout << " i " << i << " x1 " << x1[i] << " y1_abs " << y1_abs[i] << endl; 
	    y1_new[i] = y1_abs[i] * y1[i];
	    y1_new_up[i] = y1_new[i] * sqrt( pow( gth1_epps_pAl[0]->GetErrorYhigh(i) * y1_abs[i] / y1_new[i],2 )  
					     + pow(y1_abs_err[i] / y1_abs[i], 2) );	    

	    y1_new_dn[i] = y1_new[i] * sqrt( pow( gth1_epps_pAl[0]->GetErrorYlow(i) * y1_abs[i] / y1_new[i],2 )  
					     + pow(y1_abs_err[i] / y1_abs[i], 2) );	    
	  }
	...
	N1 = gth1_epps_pAu[0]->GetN();
	cout << " N Vogt " << N1 << endl;
	//double x1[3], y1[3], y1_new[3], y1_new_up[3], y1_new_dn[3];
	//double y1_abs[3], y1_abs_err[3];
	for(int i=0;i<N1; ++i)
	  {
	    int val = gth1_epps_pAu[0]->GetPoint(i, x, y);
	    x1[i] = x;
	    y1[i] = y;
	...
	y1_abs[0] = mod_pau_bu_only[0] - (mod_pau_bu_only[1] - mod_pau_bu_only[0]) / (x_new[1] - x_new[0]) * (x_new[0] - x1[0]);
	y1_abs_err[0] = mod_pau_bu_only_err[0] - (mod_pau_bu_only_err[1] - mod_pau_bu_only_err[0]) / (x_new[1] - x_new[0]) * (x_new[0] - x1[0]);

	y1_abs[1] = mod_pau_bu_only[1] + (mod_pau_bu_only[2] - mod_pau_bu_only[1]) / (x_new[2] - x_new[1]) * (x_new[2] - x1[1]);
	y1_abs_err[1] = mod_pau_bu_only_err[1] + (mod_pau_bu_only_err[2] - mod_pau_bu_only_err[1]) / (x_new[2] - x_new[1]) * (x_new[2] - x1[1]);

	y1_abs[2] = mod_pau_bu_only[3] + (mod_pau_bu_only[3] - mod_pau_bu_only[2]) / (x_new[3] - x_new[2]) * (x1[2] - x_new[3]);
	y1_abs_err[2] = mod_pau_bu_only_err[3] + (mod_pau_bu_only_err[3] - mod_pau_bu_only_err[2]) / (x_new[3] - x_new[2]) * (x1[2] - x_new[3]);

	for(int i=0;i<N;++i)
	  {
	    cout << " i " << i << " x_new " << x_new[i] << " mod_pau_bu_only " << mod_pau_bu_only[i] << endl; 
	  }
	for(int i=0;i<N1;++i)
	  {
	    cout << " i " << i << " x1 " << x1[i] << " y1_abs " << y1_abs[i] << endl; 
	    y1_new[i] = y1_abs[i] * y1[i];
	    y1_new_up[i] = y1_new[i] * sqrt( pow( gth1_epps_pAu[0]->GetErrorYhigh(i) * y1_abs[i] / y1_new[i],2 )  
					     + pow(y1_abs_err[i] / y1_abs[i], 2) );	    

	    y1_new_dn[i] = y1_new[i] * sqrt( pow( gth1_epps_pAu[0]->GetErrorYlow(i) * y1_abs[i] / y1_new[i],2 )  
					     + pow(y1_abs_err[i] / y1_abs[i], 2) );	    
	  }
	...
	N1 = gth1_epps_HeAu[0]->GetN();
	cout << " N Vogt " << N1 << endl;
	//double x1[3], y1[3], y1_new[3], y1_new_up[3], y1_new_dn[3];
	//double y1_abs[3], y1_abs_err[3];
	for(int i=0;i<N1; ++i)
	  {
	    int val = gth1_epps_HeAu[0]->GetPoint(i, x, y);
	    x1[i] = x;
	    y1[i] = y;
	...
	for(int i=0;i<N1;++i)
	  {
	    cout << " i " << i << " x1 " << x1[i] << " y1_abs " << y1_abs[i] << endl; 
	    y1_new[i] = y1_abs[i] * y1[i];
	    y1_new_up[i] = y1_new[i] * sqrt( pow( gth1_epps_HeAu[0]->GetErrorYhigh(i) * y1_abs[i] / y1_new[i],2 )  
					     + pow(y1_abs_err[i] / y1_abs[i], 2) );	    

	    y1_new_dn[i] = y1_new[i] * sqrt( pow( gth1_epps_HeAu[0]->GetErrorYlow(i) * y1_abs[i] / y1_new[i],2 )  
					     + pow(y1_abs_err[i] / y1_abs[i], 2) );	    
	  }
	...
	htmp = (TH1F*)gPad->DrawFrame(-3,0,3,1.6e-6);
	SetHistoStyle("y","B_{ll}dN/dy","",24,22);
	...
	*/
	...
	if ( bSAVE ){
		...
		c1_2->cd();
		c1_2->SaveAs("pdf/fig_RAB_y_MB_pAu.pdf");

		c1_3->cd();
		c1_3->SaveAs("pdf/fig_RAB_y_MB_dAu.pdf");
		...
		c10_2->cd();
		c10_2->SaveAs("pdf/fig_dNdy_MB_pAl.pdf");

		c10_3->cd();
		c10_3->SaveAs("pdf/fig_dNdy_MB_pAu.pdf");
	...
	TFile *outfile = new TFile("Jpsi_dNdy_pp_for_AuAu.root","recreate");
	for (int iarm=0; iarm<narm; iarm++){
		gy_pp15_sys[iarm]->Write(Form("gdNdy_sys_for_AuAu_arm%d",iarm));
		gy_pp15[iarm]->Write(Form("gdNdy_for_AuAu_arm%d",iarm));
	}
